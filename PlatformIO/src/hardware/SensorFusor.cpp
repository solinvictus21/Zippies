...
#define LIGHTHOUSE_UNLOCKED_SIGNAL_TIMEOUT      100
...
bool SensorFusor::loop(unsigned long currentTime)
{
    ...
    if (preambleBitCount < 17) {
        // SerialUSB.print("Got preamble bit: ");
        // SerialUSB.println(preambleBitCount);
    ...
    */
    ...
    switch (currentSignalState) {
        ...
        case LighthouseSignalState::SignalUnlocked:
            //estimate position updates until we have a signal lock again or until we timeout
...
void SensorFusor::calculateLighthouseData()
{
    ...
    // z = xy - yx = x1 - y0 = x
    // (-z, 0, x)
    rotationUnitVector.set(-rotationUnitVector.getZ(), 0.0, rotationUnitVector.getX(), 1.0);
...
void SensorFusor::calculatePosition()
{
    ...
    position.orientation.set(
        atan2(sensorPositions[0].getY() - sensorPositions[1].getY(),
    ...
    position.position.set(
        (BODY_CENTER_SENSOR_CENTER_OFFSET_Y * position.orientation.sin()) + ((sensorPositions[0].getX() + sensorPositions[1].getX()) / 2.0),
        (BODY_CENTER_SENSOR_CENTER_OFFSET_Y * position.orientation.cos()) + ((sensorPositions[0].getY() + sensorPositions[1].getY()) / 2.0));
    ...
    positionDelta.position.projectAlong(positionDelta.orientation.get() / 2.0);
    // */
...
void SensorFusor::estimatePosition(unsigned long currentTime)
{
    ...
    // double scale = ((double)(currentTime - positionTimeStamp)) / ((double)(positionTimeStamp - previousPositionTimeStamp));
    ...
    // positionDelta.position.multiply(scale);
    // positionDelta.orientation.set(snapAngle(positionDelta.orientation.get() * scale));
